...
void cyBot_uart_init(void)
{
    ...
    UART4_CTL_R &= 0xDE;        //disable UART
    UART4_IBRD_R = 0x8;         //set the baud rate period to 8
    UART4_FBRD_R = 0x2C;        //set the fractional baud rate period
    UART4_LCRH_R = 0x60; //set baud rate to 115200, data bits to 8, one stop bit, no parity
    UART4_CC_R = 0x0;       //set UARTCC as 0 to select system clk as the source
    UART4_CTL_R = 0x301;     //enable UART 4 AND enable UART Recive and transmit
}

void cyBot_sendByte(char data){
    while ((UART4_FR_R & 0x20) == 0x20)
        ; //wait till flag says transmit queue is full
    UART4_DR_R = data; //send data
}


int cyBot_getByte(void){
    char recived;
    while (UART4_FR_R & 0x10)
        ;  //wait till flag says receive queue is full
    recived = UART4_DR_R & 0xFF; //get data from reg
    //lcd_putc(recived);
    return recived;
}
